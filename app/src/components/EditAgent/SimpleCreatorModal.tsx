// src/components/EditAgent/SimpleCreatorModal.tsx

import React, { useState, useRef, useEffect, useCallback } from 'react';
import Modal from '@components/EditAgent/Modal';
import { SimpleTool } from '@utils/agentTemplateManager';
...
const SimpleCreatorModal: React.FC<SimpleCreatorModalProps> = ({ isOpen, onClose, onNext }) => {
  ...
  const [selectedTools, setSelectedTools] = useState<Set<SimpleTool>>(new Set());
  ...
  const resetState = useCallback(() => {
    ...
    setSelectedTools(new Set()); setConditionEnabled(false); setConditionKeyword('');
  }, []);
      ...
      <div className="flex-grow relative overflow-hidden">
        <div className="absolute inset-0 flex transition-transform duration-300 ease-in-out" style={{ transform: `translateX(-${(step - 1) * 100}%)` }}>
                        ...
                        <button key={m.name} onClick={() => setModel(m.name)} className={`relative group px-3 py-2 rounded-md border text-sm font-medium flex items-center space-x-2 transition-all duration-150 ${model === m.name ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-white text-gray-700 border-gray-300 hover:border-blue-400'}`}>
                            ...
                            {m.multimodal && (
                                <span title="Vision Model">
                                    <Eye className="h-4 w-4 text-purple-400 group-hover:text-purple-500" />
                                </span>
                            )}
                        </button>
                    ))}
          ...
          <div className="w-full flex-shrink-0 p-8 space-y-6 overflow-y-auto">
            <h3 className="text-xl font-semibold text-gray-800">Tools & Conditions</h3>
            ...
            {conditionEnabled && <input value={conditionKeyword} onChange={(e) => setConditionKeyword(e.target.value)} className="w-full p-2 bg-white border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" placeholder="e.g., 'ALERT'"/>}
            <div className="grid grid-cols-1 gap-4">
              <button onClick={() => setSelectedTools(p => p.has('notification') ? new Set([...p].filter(t => t !== 'notification')) : new Set([...p, 'notification']))} className={`group flex items-center space-x-4 p-4 border-2 rounded-lg text-left transition-all ${selectedTools.has('notification') ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-gray-400'}`}><Bell className={`h-8 w-8 transition-colors ${selectedTools.has('notification') ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-600'}`} /><div><h3 className="font-semibold text-gray-900">Get a Notification</h3><p className="text-sm text-gray-500">Sends the response as a desktop alert.</p></div></button>
              <button onClick={() => setSelectedTools(p => p.has('memory') ? new Set([...p].filter(t => t !== 'memory')) : new Set([...p, 'memory']))} className={`group flex items-center space-x-4 p-4 border-2 rounded-lg text-left transition-all ${selectedTools.has('memory') ? 'border-green-500 bg-green-50' : 'border-gray-300 hover:border-gray-400'}`}><Save className={`h-8 w-8 transition-colors ${selectedTools.has('memory') ? 'text-green-500' : 'text-gray-400 group-hover:text-gray-600'}`} /><div><h3 className="font-semibold text-gray-900">Write to Memory</h3><p className="text-sm text-gray-500">Saves the response to its own memory.</p></div></button>
            </div>
          </div>
        </div>
      </div>
