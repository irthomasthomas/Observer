...
import {
  Download,
  ChevronDown, ChevronUp, // <-- Add ChevronUp for accordions
  Eye,
  ScanText,
  Play,
  X,
...
// --- NEW HELPER COMPONENT: Mobile Tab Navigator ---
...
// --- NEW HELPER COMPONENT: Mobile Accordion for Code Tab ---
...
// --- NEW HELPER COMPONENT: Sensor Bottom Sheet ---
interface SensorSheetProps {
    isOpen: boolean;
    onClose: () => void;
    onSelect: (sensor: string) => void;
    availableAgents: { id: string; name: string }[];
}
const SensorSheet: React.FC<SensorSheetProps> = ({ isOpen, onClose, onSelect, availableAgents }) => {
    if (!isOpen) return null;

    const sensors = [
        { label: 'Screen Text', tag: '$SCREEN_OCR', icon: ScanText },
        { label: 'Screen Image', tag: '$SCREEN_64', icon: Monitor },
        { label: 'Camera', tag: '$CAMERA', icon: Camera },
        { label: 'Clipboard', tag: '$CLIPBOARD_TEXT', icon: Clipboard },
        { label: 'Microphone', tag: '$MICROPHONE', icon: Mic },
    ];

    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end" onClick={onClose}>
            <div className="w-full bg-white rounded-t-2xl p-4 shadow-xl" onClick={e => e.stopPropagation()}>
                <h3 className="text-lg font-bold text-center mb-4">Insert Sensor</h3>
                <div className="grid grid-cols-2 gap-3">
                    {sensors.map(s => (
                        <button key={s.tag} onClick={() => { onSelect(s.tag); onClose(); }} className="flex flex-col items-center justify-center p-4 bg-gray-100 rounded-lg hover:bg-gray-200">
                     ...
                     <div className="col-span-2 space-y-2">
                        {availableAgents.map(a => (
                            <button key={a.id} onClick={() => { onSelect(`$MEMORY@${a.id}`); onClose(); }} className="w-full flex items-center p-3 bg-gray-100 rounded-lg hover:bg-gray-200 text-left">
                                <Brain className="h-6 w-6 mr-3 text-green-600" />
                                <div>
                                    <span className="font-medium">Memory:</span>
                                    <span className="text-sm text-gray-600 ml-1">{a.name}</span>
                                </div>
                            </button>
                        ))}
                    </div>
...
const EditAgentModal: React.FC<EditAgentModalProps> = ({
  ...
  const [isSensorSheetOpen, setIsSensorSheetOpen] = useState(false);
  ...
  // --- We'll define the content of each section once, then place it in either the
  // --- desktop layout or the mobile tabs. This is the "Don't Repeat Yourself" principle.
  ...
  const PromptContent = () => (
    <div className="p-4 bg-white rounded-lg border border-gray-200 shadow-sm flex-grow flex flex-col h-full">
      <div className="flex justify-between items-center mb-3">
        ...
        {/* --- DESKTOP SENSOR BAR --- */}
        <div className="hidden md:flex items-center gap-x-1.5 bg-gray-100 p-1 rounded-lg border border-gray-200">
      ...
      {/* --- MOBILE SENSOR BUTTON --- */}
      <div className="md:hidden mt-4">
        <button onClick={() => setIsSensorSheetOpen(true)} className="w-full flex items-center justify-center p-3 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600">
  ...
  const CodeEditorContent = () => (
     <div className="p-4 bg-white rounded-lg border border-gray-200 shadow-sm flex-grow flex flex-col h-full">
        ...
        {/* The Logs section will be moved to its own accordion on mobile */}
   ...
   const LogsContent = () => (
    <pre className="w-full p-2 bg-gray-50 border border-gray-300 rounded-md font-mono text-xs h-48 overflow-y-auto">
      {testOutput ? (testOutput.replace(/\\n/g, '\n').split('\n').map((line, i) => (<div key={i} className={line.startsWith('ERROR:') ? 'text-red-500' : line.startsWith('SYSTEM:') ? 'text-blue-600' : line.startsWith('[DEBUG]') ? 'text-gray-500' : 'text-gray-700'}>{line}</div>))) : (<span className="text-gray-400">Logs will appear here…</span>)}
    </pre>
  );
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-grow min-h-0 bg-gray-50 overflow-y-auto">
          ...
          <div className="md:hidden p-4 space-y-4">
            ...
            {activeTab === 'prompt' && <div className="h-full flex flex-col"><PromptContent /></div>}

            {activeTab === 'code' && (
                <div className="space-y-4">
                    <Accordion title="Response Preview" icon={Play} rightContent={
                        <button onClick={handleRunModel} disabled={isRunningModel || !currentModel} className={`px-3 py-1.5 rounded-md flex items-center text-sm text-white ${isRunningModel ? 'bg-yellow-500 animate-pulse' : 'bg-green-500 hover:bg-green-600'}`}>
                           {isRunningModel ? 'Running...' : 'Run Model'}
                        </button>
                    }>
                        <CodePreviewContent />
                    </Accordion>
      ...
      {/* --- MODALS (Unchanged, but SensorSheet is new) --- */}
      <JupyterServerModal
      ...
      />
      <SensorSheet
        isOpen={isSensorSheetOpen}
        onClose={() => setIsSensorSheetOpen(false)}
        onSelect={insertSystemPromptText}
        availableAgents={availableAgentsForBlocks}
      />
    </>
  );
};
